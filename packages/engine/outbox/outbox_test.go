...
func okSender(digest string) SendFunc {
	return func(ctx context.Context, job jobs.Job, attempt jobs.RecipientAttempt, paths []string) SendOutcome {
...
func TestDispatch_OfflineRecipientBackoff(t *testing.T) {
	clk := &testClock{t: time.Date(2026, 9, 20, 6, 0, 0, 0, time.UTC)}
	calls := 0
	offline := func(ctx context.Context, job jobs.Job, attempt jobs.RecipientAttempt, paths []string) SendOutcome {
		calls++
...
func TestDispatch_RefusedIsTerminal(t *testing.T) {
	clk := &testClock{t: time.Now().UTC()}
	calls := 0
	refused := func(ctx context.Context, job jobs.Job, attempt jobs.RecipientAttempt, paths []string) SendOutcome {
		calls++
...
func TestDispatch_CancelStopsDispatch(t *testing.T) {
	clk := &testClock{t: time.Now().UTC()}
	calls := 0
	never := func(ctx context.Context, job jobs.Job, attempt jobs.RecipientAttempt, paths []string) SendOutcome {
		calls++
...
func TestDispatch_RestartRequeuesInterrupted(t *testing.T) {
	clk := &testClock{t: time.Now().UTC()}
	hang := func(ctx context.Context, job jobs.Job, attempt jobs.RecipientAttempt, paths []string) SendOutcome {
	...
	_ = hang
	calls := 0
	ob := newTestOutbox(t, clk, func(ctx context.Context, job jobs.Job, attempt jobs.RecipientAttempt, paths []string) SendOutcome {
		calls++
...
func TestRetryFailed_RequeuesWithFreshBudget(t *testing.T) {
	clk := &testClock{t: time.Now().UTC()}
	calls := 0
	failOnce := func(ctx context.Context, job jobs.Job, attempt jobs.RecipientAttempt, paths []string) SendOutcome {
		calls++
		if calls == 1 {
